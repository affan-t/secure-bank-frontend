...
import { Eye, EyeOff, Mail, Lock, User, ArrowRight, Moon, Sun, Check, Phone, Wallet, Building, CreditCard } from 'lucide-react';
...
export default function Signup() {
  ...
  const validateStep2 = () => {
    ...
    else if (!/^[0-9]{5}-[0-9]{7}-[0-9]$/.test(cnic)) {
      newErrors.cnic = 'CNIC format: 12345-1234567-1';
    }
    if (!accountNumber.trim()) newErrors.accountNumber = 'Account number is required';
    if (!accountTitle.trim()) newErrors.accountTitle = 'Account title is required';
          ...
          <div className="w-full max-w-md animate-fade-in">
            ...
            {step === 1 ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* CNIC (Required) */}
                <div className="space-y-1.5">
                  <label className="text-xs font-medium">CNIC <span className="text-destructive">*</span></label>
                  <input
                    type="text"
                    value={cnic}
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className={cn(
                      'w-full h-11 px-3 rounded-lg bg-secondary/50 border text-sm transition-all duration-300',
                      errors.accountNumber ? 'border-destructive' : 'border-transparent focus:border-primary'
                    )}
                  />
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-medium">Account Title <span className="text-destructive">*</span></label>
                  <input
                    type="text"
                    value={accountTitle}
                    ...
                    className={cn(
                      'w-full h-11 px-3 rounded-lg bg-secondary/50 border text-sm transition-all duration-300',
                      errors.accountTitle ? 'border-destructive' : 'border-transparent focus:border-primary'
                    )}
                  />
                  {errors.accountTitle && <p className="text-xs text-destructive">{errors.accountTitle}</p>}
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={isLoading || !username.trim() || !phone.trim() || !cnic.trim() || !accountNumber.trim() || !accountTitle.trim() || !agreeTerms}
                    className={cn(
                      'flex-1 h-11 rounded-lg font-medium flex items-center justify-center gap-2 transition-all duration-300',
                      'gradient-accent text-white hover:opacity-90',
                      (!username.trim() || !phone.trim() || !cnic.trim() || !accountNumber.trim() || !accountTitle.trim() || !agreeTerms || isLoading) && 'opacity-50 cursor-not-allowed hover:opacity-50'
                    )}
                  >
                    {isLoading ? (
